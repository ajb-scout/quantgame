...
use ratatui::{layout::Rect, text::Line, widgets::{block::Title, Block, Paragraph}, Frame};
...
pub(crate) fn display_result_summary(frame: &mut Frame, area: Rect, game: &MathGame) {
    ...
    let grouped_sums: HashMap<Sign, i32> = game.answers.clone().into_iter().fold(HashMap::new(), |mut acc, item| {
    ...
    frame.render_widget(Paragraph::new(line_vec).block(Block::bordered().title(title)), area);
}
